...
class NotesDetailsViewController: UIViewController  , UITableViewDelegate,UITableViewDataSource{
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.upperViewHeightConstraint.constant = self.leftBottomLabel.frame.origin.y + self.leftBottomLabel.frame.size.height + 20
    }
...
func setUpUIUpperView()
{
    ...
    self.upperViewThirdLabel.text = notesObj.desc
   ...
   // self.upperImage.image = UIImage(named: "live_class1.PNG")
    ...
    if(type == 2 || type == 3){
        let total = (notesObj.cnt_completed.intValue) + (notesObj.cnt_not_completed.intValue)
        self.rightUpperLabel.text  =  "\(notesObj.cnt_completed) Out of \(total)"
        if(type == 2)
        {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lastVisitedLabel.text = "last Visited: " + (NotesDetailObj?.student_view_info[indexPath.row] as! Student_view_info).last_visited
        ...
        cell.completedLabel.text = "\((NotesDetailObj?.student_view_info[indexPath.row] as! Student_view_info).completion_percentage)" + "% Completed"
        drawLineFromPoint(CGPoint(x: Double(cell.percentageView.frame.size.width - 10) * (0/100), y: 20.0), ofColor: DataUtils.colorWithHexString("3bab14"), inView: (cell.percentageView), String(describing: (NotesDetailObj?.student_view_info[indexPath.row] as! Student_view_info).completion_percentage))
         cell.upperImageView.sd_setImage(with: URL(string: (NotesDetailObj?.student_view_info[indexPath.row] as! Student_view_info).pic))
    ...
    @IBAction func readButtonTapped(_ sender: Any) {
        ...
        {
        let addedWidth:CGFloat = DataUtils.getDynamicHeight(self.upperViewThirdLabel.text!, width: tableView.frame.size.width - 40)
            self.viewHeightConstraint.constant = 85 + addedWidth
            self.upperViewHeightConstraint.constant = upperViewHeightConstraint.constant + self.viewHeightConstraint.constant
            isExpanded = true
