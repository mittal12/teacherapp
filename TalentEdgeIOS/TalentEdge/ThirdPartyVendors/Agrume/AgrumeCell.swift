...
final class AgrumeCell: UICollectionViewCell {
  ...
  fileprivate lazy var scrollView: UIScrollView = {
    let scrollView = UIScrollView(frame: CGRect(x: 0, y: 74, width: self.contentView.frame.size.width, height: self.contentView.frame.size.height-104))
  ...
  fileprivate lazy var imageView: UIImageView = {
    let imageView = UIImageView(frame: CGRect(x: 0, y: 74, width: self.contentView.frame.size.width, height: self.contentView.frame.size.height-104))
  ...
  override init(frame: CGRect) {
    ...
    var viewedIndexArray : [Int]!
    var notesObject : TENotes!
...
//    dismiss()
  ...
  func updateScrollViewAndImageViewForCurrentMetrics() {
    ...
    //print(contentInsetForScrollView(atScale: scrollView.zoomScale))
    scrollView.contentInset = contentInsetForScrollView(atScale: 1)
    
//        contentInsetForScrollView(atScale: scrollView.zoomScale)
  }

  fileprivate func resizedFrameForSize(_ imageSize: CGSize) -> CGRect {
    var frame = (frame: CGRect(x: 0, y: 74, width: self.contentView.frame.size.width, height: self.contentView.frame.size.height-104))
